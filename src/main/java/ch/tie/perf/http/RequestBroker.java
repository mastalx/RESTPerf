...
public class RequestBroker implements Closeable {
  ...
  public RequestBroker(String iengineUser, String serviceUser, String servicePassword, Statistics statistics) {
    ...
    restClient = createRestClient(Optional.of(new UsernamePasswordCredentials(serviceUser, servicePassword)));
  }
  ...
  private <T> T doRequest(final HttpRequestBase request, final Class<T> resultClass, String scenarioName) {
    ...
    try (CloseableHttpResponse response = restClient.execute(request)) {
      ...
      LOGGER.debug("got response with Id: {}, Response Status: {}", requestId, statusLine);
